...
<script>
import { storage, auth,moviesCollection } from '../includes/firebase.js';
export default {
    ...
    methods: {
        ...
        upload($event) {
            ...
            files.forEach((file) => {
                ...
                const songsRef = storageRef.child(`movies/${file.name}`);
             ...
             const uploadIndex =  this.uploads.push({
               ...
               task.on('state_changed', (snapshot) => {
                 ...
                 movie.url = await task.snapshot.ref.getDownloadURL();
               const movieRef = await moviesCollection.add(movies);
               const movieSnapshot = await movieRef.get();

               this.addMovie(movieSnapshot);
